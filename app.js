...
function renderMore() {
  const areaItems=navItems.filter(([id])=>['fornecedores','inspiracao','mesas','grande-dia','memorias','edicoes'].includes(id));
  const areaCards=areaItems.map(([id,label,ico])=>`<button class="card more-menu-card" type="button" data-nav="${id}"><span class="more-menu-icon">${icon(ico)}</span><span class="more-menu-copy"><strong>${label}</strong><small>${id==='edicoes'?`Edição atual: ${state.plan==='premium'?'Premium':'Comercial'}.`:'Abrir esta área da agenda.'}</small></span><span class="more-menu-arrow">›</span></button>`).join('');
  return `<section class="more-menu-section"><div class="more-menu-heading"><p class="eyebrow">OUTRAS ÁREAS</p><h2>Mais opções</h2></div><div class="more-menu-grid">${areaCards}</div></section>
    <section class="more-menu-section"><div class="more-menu-heading"><p class="eyebrow">CONTA E APLICAÇÃO</p><h2>Definições</h2></div><div class="more-menu-grid">
      <button class="card more-menu-card" type="button" data-action="account"><span class="more-menu-icon">${icon('guests')}</span><span class="more-menu-copy"><strong>Conta e sincronização</strong><small>Entrar e editar os dados do casamento.</small></span><span class="more-menu-arrow">›</span></button>
      <button class="card more-menu-card" type="button" data-action="export-data"><span class="more-menu-icon">${icon('download')}</span><span class="more-menu-copy"><strong>Exportar dados</strong><small>Guardar uma cópia de segurança.</small></span><span class="more-menu-arrow">›</span></button>
      <button class="card more-menu-card" type="button" data-action="privacy"><span class="more-menu-icon">${icon('day')}</span><span class="more-menu-copy"><strong>Privacidade e dados</strong><small>Consultar ou eliminar os teus dados.</small></span><span class="more-menu-arrow">›</span></button>
      <button class="card more-menu-card" type="button" data-nav="admin"><span class="more-menu-icon">${icon('sparkles')}</span><span class="more-menu-copy"><strong>Administração — demo</strong><small>Abrir o backoffice do negócio.</small></span><span class="more-menu-arrow">›</span></button>
      <button class="card more-menu-card more-menu-reset" type="button" data-action="reset-data"><span class="more-menu-icon">${icon('more')}</span><span class="more-menu-copy"><strong>Repor demonstração</strong><small>Voltar aos dados iniciais do protótipo.</small></span><span class="more-menu-arrow">›</span></button>
    </div></section>`;
}
...
$('#mobile-menu').addEventListener('click',()=>setMainMenu(!$('.sidebar').classList.contains('open')));
$('#sidebar-overlay').addEventListener('click',()=>setMainMenu(false));
document.addEventListener('click',event=>{
  if (event.target.closest('[data-nav]')) setMainMenu(false);
  else if ($('.sidebar').classList.contains('open')&&!event.target.closest('.sidebar')&&!event.target.closest('#mobile-menu')) setMainMenu(false);
});
document.addEventListener('keydown',event=>{if(event.key==='Escape'&&$('.sidebar').classList.contains('open'))setMainMenu(false)});
